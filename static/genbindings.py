...
def main():
    ...
    if platform == 'win32':
        x86_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.4/prebuilt', '%s' % cur_platform))
        if not os.path.exists(x86_llvm_path):
            x86_llvm_path = os.path.abspath(os.path.join(
                ndk_root, 'toolchains/llvm-3.3/prebuilt', '%s' % cur_platform))
    else:
        x86_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.4/prebuilt', '%s-%s' % (cur_platform, 'x86')))
        if not os.path.exists(x86_llvm_path):
            x86_llvm_path = os.path.abspath(os.path.join(
                ndk_root, 'toolchains/llvm-3.3/prebuilt', '%s-%s' % (cur_platform, 'x86')))

    x64_llvm_path = os.path.abspath(os.path.join(
        ndk_root, 'toolchains/llvm-3.4/prebuilt', '%s-%s' % (cur_platform, 'x86_64')))
    if not os.path.exists(x64_llvm_path):
        x64_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.3/prebuilt', '%s-%s' % (cur_platform, 'x86_64')))

    if not os.path.exists(x64_llvm_path):
        x64_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.5/prebuilt', '%s-%s' % (cur_platform, 'x86_64')))

    if not os.path.exists(x64_llvm_path):
        x64_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.6/prebuilt', '%s-%s' % (cur_platform, 'x86_64')))

    if not os.path.exists(x64_llvm_path):
        x64_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm/prebuilt', '%s-%s' % (cur_platform, 'x86_64')))
    ...
    if os.path.isdir(x86_llvm_path):
        ...
        print('path: %s or path: %s are not valid! ') % (
    ...
    config.set('DEFAULT', 'frameworks', frameworks)
    ...
    conf_ini_file = os.path.abspath(os.path.join(
        os.path.dirname(__file__), 'userconf.ini'))
    ...
    try:
        ...
        cmd_args = {
            ...
            # 'cocos2dx_curve.ini':('cocos2dx_curve', 'lua_cocos2dx_curve_auto'),\
            # 'cocos2dx_cmdarrow.ini':('cocos2dx_cmdarrow', 'lua_cocos2dx_cmdarrow_auto'),\
            # 'cocos2dx_lead.ini':('cocos2dx_lead', 'lua_cocos2dx_lead_auto'),\
            # 'cocos2dx_puzzle.ini':('cocos2dx_puzzle', 'lua_cocos2dx_puzzle_auto'),\
            # 'cocos2dx_multitexture.ini':('cocos2dx_multitexture', 'lua_cocos2dx_multitexture_auto'),
            # 'cocos2dx_maputil.ini':('cocos2dx_maputil', 'lua_cocos2dx_maputil_auto'),\
            # 'cocos2dx_movieplayer.ini':('cocos2dx_movieplayer', 'lua_cocos2dx_movieplayer_auto'),\
            # 'cocos2dx_experimental_audio_engine.ini':('cocos2dx_experimental_audio_engine', 'lua_cocos2dx_experimental_audio_engine_auto'),\
        }
        target = 'lua'
